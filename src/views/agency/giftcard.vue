<template>
  <div class="post-container">
    ...
    <template class="brand">
      <el-radio v-model="brand" label="N">Nike</el-radio>
      <el-radio v-model="brand" label="A">Adidas</el-radio>
      <el-radio v-model="brand" label="JD">JDSports</el-radio>
    </template>
    ...
    <el-table
      ...
      style="width: 100%"
      height="75vh">
      <el-table-column label="编号" prop="giftcard_ID"></el-table-column>
      <el-table-column label="礼品卡卡号" prop="card_num"></el-table-column>
      <el-table-column label="PIN码" prop="pin"></el-table-column>
      <el-table-column label="余额" prop="balance"></el-table-column>
      <el-table-column label="品牌" prop="brand"><template>Nike</template></el-table-column>
      <el-table-column label="操作" align="right" width="200">
        <template slot="header">
          <el-button
            size="mini"
            type="primary"
            @click="dialogAddVisible=true;newItem={card_num: '',pin: '',balance: '',brand: '',};">新增</el-button>
        </template>
    ...
    <el-dialog :title='"新增"+name' :visible.sync="dialogAddVisible">
      <el-form size="mini">
        ...
        <el-form-item label="品牌">
          <el-input v-model="newItem.brand"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :title='"修改"+name' :visible.sync="dialogEditVisible">
      <el-form>
...
<script>
  ...
  import { addGiftcard,delGiftcard,getGiftcard } from '@/network/agency.js'
  export default {
    name: "Giftcard",
    data () {
      return {
    ...
    methods:{
      ...
      handleAdd() {
        if(this.newItem.card_num=='') {
          this.$message({type: 'warning',message: '请填写卡号'});
        } else if(this.newItem.pin=='') {
          this.$message({type: 'warning',message: '请填写PIN码'});
        } else if(this.newItem.balance=='') {
          this.$message({type: 'warning',message: '请填写余额'});
        } else if(this.newItem.brand=='') {
          this.$message({type: 'warning',message: '请填写品牌'});
        } else {
          addGiftcard(this.newItem).then(res=>{
            if(res.data.status=='200') {
              this.dialogAddVisible = false;
              this.currentPage = 1;
              this._getList(this.currentPage);
              this.$message({type: 'success',message: '添加成功'});
              this.newItem = {card_num: '',pin: '',balance: '',brand: '',};
            } else {
